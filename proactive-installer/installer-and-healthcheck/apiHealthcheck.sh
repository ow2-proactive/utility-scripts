...
#input parameters
#the content of the file given as input to the installation script
inputConfig=$(cat inputConfig.json | jq '.')
#the passphrase to decrypt the passwords
passphrase=$(echo $inputConfig | jq -r '.serverConfiguration.passphrase')
proactiveAdminPassword=$(echo $inputConfig | jq -r '.serverConfiguration.proactiveAdminPassword')
#decrypted password of the user connecting to the ProActive Portals
...
serverDNSName=$(echo $inputConfig | jq -r '.serverConfiguration.serverDNSName')
#server Port
webPort=$(echo $inputConfig | jq -r '.serverConfiguration.webPort')
#web protocol, http or https
webProtocol=$(echo $inputConfig | jq -r '.serverConfiguration.webProtocol')
#name of one of the node sources, needed in a test to get its description
nsName=$(cat inputConfig.json | jq -r '.nodesConfiguration[0]' | jq -r '.name')
