...
sudo apt-get install -y kubectl  || { log_print ERROR "kubectl installation failed!"; exit $EXITCODE; }
sudo apt-get install -y kubelet  || { log_print ERROR "kubelet installation failed!"; exit $EXITCODE; }
